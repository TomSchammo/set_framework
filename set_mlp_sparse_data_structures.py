...
def createSparseWeights(epsilon, noRows, noCols):
    ...
    for i in range(epsilon * (noRows + noCols)):
        weights[np.random.randint(0, noRows),
                np.random.randint(0, noCols)] = np.float64(np.random.randn() /
                                                           10)
...
class SET_MLP:
    ...
    def __init__(self, dimensions, activations, epsilon=20):
        ...
        for i in range(len(dimensions) - 1):
            self.w[i + 1] = createSparseWeights(
    ...
    def _update_w_b(self, index, dw, delta):
        ...
        if (index not in self.pdw):
            ...
            self.pdd[index] = self.momentum * self.pdd[
                index] - self.learning_rate * np.mean(delta, 0)
    ...
    def fit(self,
        ...
        for i in range(epochs):
            ...
            print("\nSET-MLP Epoch ", i)
            print("Training time: ", t2 - t1, "; Loss train: ",
                  losstrain / (x.shape[0] // batch_size))

            # test model performance on the test data at each epoch
            # this part is useful to understand model performance and can be commented for production settings
            if (testing):
                t3 = datetime.datetime.now()
                accuracy, activations = self.predict(x_test, y_test,
                                                     batch_size)
                ...
                print("Testing time: ", t4 - t3, "; Loss test: ", losstest,
            ...
            if (i < epochs - 1
                ...
                self.weightsEvolution_Amar(
                )  #this implementation has the same behaviour as the original one, but it is much faster.
    ...
    def weightsEvolution(self):
        ...
        for i in range(1, self.n_layers):
            ...
            smallestPositive = values[int(
                min(values.shape[0] - 1, lastZeroPos + self.zeta *
                    (values.shape[0] - lastZeroPos)))]
    ...
    def weightsEvolution_Amar(self):
        ...
        for i in range(1, self.n_layers):
            ...
            valsWNew = valsW[(valsW > smallestPositive) |
                             (valsW < largestNegative)]
            rowsWNew = rowsW[(valsW > smallestPositive) |
                             (valsW < largestNegative)]
            colsWNew = colsW[(valsW > smallestPositive) |
                             (valsW < largestNegative)]

            newWRowColIndex = np.stack((rowsWNew, colsWNew), axis=-1)
            oldPDRowColIndex = np.stack((rowsPD, colsPD), axis=-1)

            newPDRowColIndexFlag = array_intersect(
                oldPDRowColIndex, newWRowColIndex)  # careful about order
            ...
            while (lengthRandom > 0):
                ...
                randomWRowColIndex = np.unique(
                    randomWRowColIndex,
                    axis=0)  # removing duplicates in new rows&cols
                oldWRowColIndex = np.stack((rowsWNew, colsWNew), axis=-1)

                uniqueFlag = ~array_intersect(
                    randomWRowColIndex,
                    oldWRowColIndex)  # careful about order & tilda
