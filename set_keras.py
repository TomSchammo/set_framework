...
class SET_MLP_CIFAR10:
    def __init__(self, strategy: BaseSETStrategy, max_epochs):
        if inspect.isclass(strategy):
            raise TypeError(f"Expected strategy instance, got class {strategy.__name__}. Did you forget ()?")
    ...
    def create_model(self):
        ...
        use_skip = isinstance(self.strategy, (FisherDiagonalSkipSET, NeuronCentralitySkipSET, NeuronEMASkipSET))
    ...
    def _call_prune(self, mask_buffer, weights_flat, mask_flat, extra_info):
        ...
        for attempt in (
            ...
            try:
                ...
                continue
        try:
            return fn(mask_buffer=mask_buffer, weight_values=weights_flat, weight_positions=mask_flat, extra_info=extra_info)
        except TypeError:
            pass
        raise TypeError(f"Could not call prune_neurons() for {self.strategy.__class__.__name__}")
    ...
    def _call_regrow(self, noRewires, shape, mask_buffer, extra_info):
        ...
        for attempt in (
            ...
            try:
                ...
                continue
        try:
            return fn(num_to_add=noRewires, dimensions=shape, mask=mask_buffer, extra_info=extra_info)
        except TypeError:
            pass
        raise TypeError(f"Could not call regrow_neurons() for {self.strategy.__class__.__name__}")

        
    def rewireMask(self, weights, noWeights, mask_buffer, core_buffer, extra_info=None,
                   fisher_g=None, fisher_v=None):
        """
        BUFFER MINDSET:
        ...
        out = self._call_prune(mask_buffer, weights.ravel(), mask_buffer.ravel(), prune_extra)
    ...
    def _prune_only_into_buffers(self, weights, noWeights, mask_buffer, core_buffer, extra_info=None, fisher_v=None):
        """
    ...
    def fisher_choose_between_W2_and_skip(self, g2, v2, gskip, vskip):
        ...
        need2 = self._prune_only_into_buffers(
            weights=self.w2[0], noWeights=self.noPar2,
            mask_buffer=self.wm2_buffer, core_buffer=self.wm2_core_buffer,
            fisher_v=v2
        )
        needs = self._prune_only_into_buffers(
            weights=self.wSkip02[0], noWeights=self.noParSkip02,
            mask_buffer=self.wmSkip02_buffer, core_buffer=self.wmSkip02_core_buffer,
            fisher_v=vskip
        )
    ...
    def weightsEvolution(self, fisher_payload=None):
        ...
        if name == "RandomSET":
            self.rewireMask(self.w1[0], self.noPar1, self.wm1_buffer, self.wm1_core_buffer, extra_info={"temp_buf": self.wm1_core_buffer})
            self.rewireMask(self.w2[0], self.noPar2, self.wm2_buffer, self.wm2_core_buffer, extra_info={"temp_buf": self.wm2_core_buffer})
            self.rewireMask(self.w3[0], self.noPar3, self.wm3_buffer, self.wm3_core_buffer, extra_info={"temp_buf": self.wm3_core_buffer})

        elif name == "NeuronCentralitySET":
            self.rewireMask(self.w1[0], self.noPar1, self.wm1_buffer, self.wm1_core_buffer, extra_info={"layer": "layer_1", "self": self})
            self.rewireMask(self.w2[0], self.noPar2, self.wm2_buffer, self.wm2_core_buffer, extra_info={"layer": "layer_2", "self": self})
            self.rewireMask(self.w3[0], self.noPar3, self.wm3_buffer, self.wm3_core_buffer, extra_info={"layer": "layer_3", "self": self})

        elif name == "NeuronEMASet":
            self.rewireMask(self.w1[0], self.noPar1, self.wm1_buffer, self.wm1_core_buffer, extra_info={"layer": "layer_1", "self": self})
            self.rewireMask(self.w2[0], self.noPar2, self.wm2_buffer, self.wm2_core_buffer, extra_info={"layer": "layer_2", "self": self})
            self.rewireMask(self.w3[0], self.noPar3, self.wm3_buffer, self.wm3_core_buffer, extra_info={"layer": "layer_3", "self": self})

        elif name == "FisherDiagonalSET":
            if fisher_payload is None:
                raise ValueError("FisherDiagonalSET requires fisher_payload.")
            g1, g2, _, g3 = fisher_payload["grads"]
            v1, v2, _, v3 = fisher_payload["Vs"]

            self.rewireMask(self.w1[0], self.noPar1, self.wm1_buffer, self.wm1_core_buffer, fisher_g=g1, fisher_v=v1)
            self.rewireMask(self.w2[0], self.noPar2, self.wm2_buffer, self.wm2_core_buffer, fisher_g=g2, fisher_v=v2)
            self.rewireMask(self.w3[0], self.noPar3, self.wm3_buffer, self.wm3_core_buffer, fisher_g=g3, fisher_v=v3)

        elif name == "FisherDiagonalSkipSET":
            if fisher_payload is None:
            ...
            self.fisher_choose_between_W2_and_skip(g2=g2, v2=v2, gskip=gskip, vskip=vskip)

        elif name == "NeuronCentralitySkipSET":
        ...
        elif name == "NeuronEMASkipSET":
            # W1 & W3 normal EMA
            self.rewireMask(self.w1[0], self.noPar1, self.wm1_buffer, self.wm1_core_buffer,
                            extra_info={"layer": "layer_1", "self": self})
            self.rewireMask(self.w3[0], self.noPar3, self.wm3_buffer, self.wm3_core_buffer,
                            extra_info={"layer": "layer_3", "self": self})
    ...
    def train(self, target_accuracy=1.0):
        ...
        for epoch in range(0, self.maxepoches):
            ...
            if isinstance(self.strategy, FisherDiagonalSET):
                bs = 256
                g1, g2, g3 = self.compute_kernel_grads_no_skip(x_train[:bs], y_train[:bs])
                ...
                fisher_payload = {"grads": (g1, g2, None, g3),
                                  "Vs":    (self.strategy.v1, self.strategy.v2, None, self.strategy.v3)}

            if isinstance(self.strategy, FisherDiagonalSkipSET):
                bs = 256
                g1, g2, gskip, g3 = self.compute_kernel_grads_with_skip(x_train[:bs], y_train[:bs])
                b = self.strategy.beta
                self.strategy.v1 = b * self.strategy.v1 + (1.0 - b) * (g1.astype(np.float32) ** 2)
                self.strategy.v2 = b * self.strategy.v2 + (1.0 - b) * (g2.astype(np.float32) ** 2)
                self.strategy.vSkip02 = b * self.strategy.vSkip02 + (1.0 - b) * (gskip.astype(np.float32) ** 2)
                self.strategy.v3 = b * self.strategy.v3 + (1.0 - b) * (g3.astype(np.float32) ** 2)
                fisher_payload = {"grads": (g1, g2, gskip, g3),
